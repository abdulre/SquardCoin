...
PACKAGE=SQC-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=SQC-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} SquardCoin.exe SquardCoinservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe SquardCoin.exe
cp installer/lib/JavaExe.exe SquardCoinservice.exe
cp -a ${FILES} nxt
...
/Library/Java/JavaVirtualMachines/jdk1.8.0_66.jdk/Contents/Home/bin/javapackager -deploy -outdir . -outfile SQC-client -name SQC-installer -width 34 -height 43 -native dmg -srcfiles ${PACKAGE}.jar -appclass com.izforge.izpack.installer.bootstrap.Installer -v -Bmac.category=Business -Bmac.CFBundleIdentifier=org.nxt.client.installer -Bmac.CFBundleName=SQC-Installer -Bmac.CFBundleVersion=${MACVERSION} -BappVersion=${MACVERSION} -Bicon=installer/AppIcon.icns -Bmac.signing-key-developer-id-app="Developer ID Application: Stichting NXT (YU63QW5EFW)" > installer/javapackager.log 2>&1
